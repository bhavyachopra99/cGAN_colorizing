...
def load_and_preprocess_data(split='train', batch_size=32, num_samples=3000):
    """
    ...
    with tqdm(total=total_samples, desc=f"Preprocessing {split} data") as pbar:
        ...
        dataset = dataset.batch(batch_size).shuffle(3000).prefetch(tf.data.AUTOTUNE)
...
def generate_and_save_images(model, epoch, test_input, output_dir='output_3000', max_images=16):
...
def plot_losses(csv_path, output_dir='output_3000'):
    epochs = []
    gen_losses = []
    disc_losses = []
...
def plot_metrics(csv_path, output_dir='output_3000'):
    epochs = []
    ssim_scores = []
    psnr_scores = []
...
def main():
    ...
    train_dataset = load_and_preprocess_data(split='train', batch_size=32, num_samples=3000)
